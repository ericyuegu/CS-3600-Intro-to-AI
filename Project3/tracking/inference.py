...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # update beliefs for each particle
        for location, val in beliefs.items():
        	dist = util.manhattanDistance(location, pacmanPosition)
        	beliefs[location] = val * emissionModel[dist]
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        # update joint beliefs based on noisy observations
        for p in self.particles:
        	prob = 1.0
        	for ghost in range(0, self.numGhosts):
        		if noisyDistances[ghost] != None:
        			ghostPos = p[ghost]
        			distance = util.manhattanDistance(ghostPos, pacmanPosition)
        			prob = prob * emissionModels[ghost][distance]
        		else:
        			p = self.getParticleWithGhostInJail(p, ghost)
        	beliefs[p] += prob

        # all particles receive zero weight, resample from prior to recover
        if all(value == 0 for value in beliefs.values()):
        	self.initializeParticles()
