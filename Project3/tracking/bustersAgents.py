...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        positions = [] # likeliest position for each ghost
        posprobabilities = []

        for ghostDistribution in livingGhostPositionDistributions:
            # current highest likelihood position for this ghost
            likeliestGhostPos = None
            likeliestGhostProb = 0
            for pos, prob in ghostDistribution.items():
                if prob > likeliestGhostProb:
                    likeliestGhostProb = prob
                    likeliestGhostPos = pos

            positions.append(likeliestGhostPos)
        ...
        # find action that brings Pacman closer to closest ghost
        shortestActionDist = float("inf")
        bestAction = None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            newDist = self.distancer.getDistance(closestGhostPos, successorPosition)
            if newDist < shortestActionDist:
                shortestActionDist = newDist
                bestAction = action
